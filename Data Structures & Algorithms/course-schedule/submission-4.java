class Solution {
    ...
    // public boolean canFinish(int numCourses, int[][] prerequisites) {
    //   Map<Integer, Set<Integer>> graph = new ArrayList();
    //   Map<Integer, Set<Integer>> indegree = new HasMap();
    ...
    //   for(int[] course : prerequisites) {
    //     graph.putIfAbsent(course[0],  new Hashset<>());
    //     graph.get(course[0]).add(course[1]);
    //     indegree.putIfAbsent(course[1], new Hashset<>());
    //     indegree.get(course[1]).add(course[0]);
    //   }
